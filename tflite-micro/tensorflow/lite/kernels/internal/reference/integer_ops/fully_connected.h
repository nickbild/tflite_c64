...
inline void FullyConnected(
    ...
    const int32_t* bias_data, const RuntimeShape& output_shape,
    int8_t* output_data) {
      //printf("NAB FULLY CONNECTED EXEC\n");
  const int32_t input_offset = params.input_offset;
  ...
  //printf("NAB FULLY CONNECTED EXEC BATCHES %d\n", batches);
  //printf("NAB FULLY CONNECTED EXEC OUTPUT DEPTH %d\n", output_depth);
  //printf("NAB FULLY CONNECTED EXEC ACCUM DEPTH %d\n", accum_depth);
  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      ...
      for (int d = 0; d < accum_depth; ++d) {
        ...
        printf("NAB FULLY CONNECTED EXEC INPUT VAL %d\n", input_val);
        printf("NAB FULLY CONNECTED EXEC FILTER VAL %d\n", filter_val);
        printf("NAB FULLY CONNECTED EXEC FILTER OFFSET %d\n", filter_offset);
        printf("NAB FULLY CONNECTED EXEC INPUT OFFSET %d\n", input_offset);


        acc += (filter_val + filter_offset) * (input_val + input_offset);
      }
      if (bias_data) {
        printf("NAB FULLY CONNECTED EXEC BIAS DATA %d\n", bias_data[out_c]);
        acc += bias_data[out_c];
      }

      //printf("NAB FULLY CONNECTED EXEC ACC 1 %d\n", acc);

      printf("NAB FULLY CONNECTED OUTPUT MULTIPLIER %d\n", output_multiplier);
      //printf("NAB FULLY CONNECTED OUTPUT SHIFT %d\n", output_shift);
      int right_shift = output_shift > 0 ? 0 : -output_shift;
      printf("NAB FULLY CONNECTED RIGHT SHIFT %d\n", right_shift);

      acc = MultiplyByQuantizedMultiplier(acc, output_multiplier, output_shift);
      //printf("NAB FULLY CONNECTED EXEC ACC 2 %d\n", acc);
      printf("NAB FULLY CONNECTED EXEC OUTPUT OFFSET %d\n", output_offset);
      acc += output_offset;
      //printf("NAB FULLY CONNECTED EXEC ACT MIN %d\n", output_activation_min);
      //printf("NAB FULLY CONNECTED EXEC ACT MAX %d\n", output_activation_max);
      acc = std::max(acc, output_activation_min);
      acc = std::min(acc, output_activation_max);
      //printf("NAB FULLY CONNECTED EXEC ACC 3 %d\n", acc);
      //printf("NAB FULLY CONNECTED EXEC OUTPUT POS %d\n", out_c + output_depth * b);
      output_data[out_c + output_depth * b] = static_cast<int8_t>(acc);
    }
  }
}
